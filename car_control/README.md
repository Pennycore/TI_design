...
MSPM0G3507 使用 3.3V IO，PB2 不能直接承受 5V DAT。传感器连接前应安装
`PULL` 开漏模式跳线帽，然后重新给传感器上电；DAT 由主控内部上拉至
3.3V，CLK 保持推挽输出。

灰度传感器可以使用 LaunchPad J10 的5V供电，但电机VM必须使用独立的
电机电源。LaunchPad、灰度传感器、TB6612和电机电源负极必须共地。

## 灰度数据

`GraySensor_Read()` 通过 CLK/DAT 返回八路数字量：

- bit0～bit7 对应第 1～8 路探头
- 接近白场输出 1
- 接近黑场输出 0

传感器模块自身完成黑白阈值校准，MSPM0只读取校准后的0/1结果。CLK空闲
保持高电平；每次下降沿读取当前通道，随后上升沿切换到下一通道。
